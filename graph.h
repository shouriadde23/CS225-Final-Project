 ...
 *  - Added doxygen comments
 *  - Created better error handling
 *  - More encapsulated and object-orientede
 *
...
public:
    ...
    // helper function to load in data from text file; used to minimize number of nodes & edges
    std::vector<std::string> loadSmallerGraphData(const std::string & filename);
    // helper function to load in all data, does not filter out any nodes/edges
    vector<string> loadAllGraphData(const string & filename);

    // function that connects source and dest vertices; updates adjancency list
    void buildGraph(const vector<string> & vect);

    // public getter for adjacency_list
    unordered_map<Vertex, unordered_map<Vertex, Edge>> getAdjacencyList();
